...
import org.hammerlab.guacamole.reference.{ContigIterator, ContigName, HasLocus, Locus, Position, ReferenceRegion}
...
abstract class PositionRegionsIteratorBase[R <: ReferenceRegion, T <: HasLocus, U](loci: LociSet,
  ...
  var curContig: ContigName = _

  def clearContig(): Unit = {
    curContigLociIterator = null
    while (regions.hasNext && regions.head.contigName == curContig) {
      regions.next()
    }
  }
  ...
  override def _advance: Option[(Position, U)] = {
    ...
    if (curContigLociIterator == null) {
      ...
      curContig = regions.head.contigName

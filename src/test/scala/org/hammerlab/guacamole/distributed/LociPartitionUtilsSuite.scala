...
import org.hammerlab.guacamole.loci.partitioning.LociPartitioner.{NumPartitions, PartitionIdx}
import org.hammerlab.guacamole.loci.partitioning.{ApproximatePartitioner, ApproximatePartitionerArgs, UniformPartitioner}
...
class LociPartitionUtilsSuite extends GuacFunSuite {
  test("partitionLociUniformly") {
    ...
    val expectedBuilder4 = LociMap.newBuilder[PartitionIdx]
    for (i <- 0 until 100) {
